"""
Batch evaluation runner for the Neur2BiLO WatwaOS adapter.
Runs 05_run_ml_blo across all instances in parallel or sequentially and aggregates results.

Usage:
    python -m blo.scripts.evaluate_watwa_batch --problem watwa_v1 --n_procs 4
"""
...
def x_to_scenario_key(x):
    """Converts a decision array to a scenario tuple string key."""
    return str(tuple(x))


def evaluate_single(inst_idx, program_dir, result_path):
    """Loads ML inference output and compares metrics against WatwaOS ground truth."""
    opt_result_path = os.path.join(program_dir, "build", "optimize-result.json")
    if not os.path.exists(result_path) or not os.path.exists(opt_result_path):
        return None

    with open(result_path, "rb") as f:
        res = pickle.load(f)

    data = load_json(opt_result_path)
    ...
    # Greedy baseline: all HighFreq (alternative 2)
    ...
    if greedy_key in solutions:
        ...
        greedy_energy = max(v["energy"] for v in solutions.values())
...
def run_instance(args_tuple):
    """Executes 05_run_ml_blo for a single instance index, resuming if results exist."""
    ...
    existing = [
        ...
        if f.endswith(".pkl")
        and (f"i-{inst_idx}_" in f or f.endswith(f"i-{inst_idx}.pkl") or f"_i-{inst_idx}" in f)
    ]
    if existing:
        return True
    ...
    if result.returncode != 0:
        print(f"  [!] Instance {inst_idx} failed: {result.stderr[-200:]}")
        return False
    return True
...
def main(args):
    ...
    print("Running ML model inference across instances...\n")

    extra_args = [
        "--use_attention",
        str(args.use_attention),
        "--attention_num_heads",
        str(args.attention_num_heads),
        "--use_context",
        str(args.use_context),
        "--context_hidden_dim",
        str(args.context_hidden_dim),
    ]
    ...
    if args.n_procs > 1:
        ...
        for i, task in enumerate(tasks):
            ...
            results.append(ok)
            if (i + 1) % 10 == 0:
                print(f"  {i + 1}/{n} instances completed ({time.time() - t0:.0f}s)")

    n_failed = sum(1 for r in results if not r)
    print(f"\nInference completed: {n - n_failed}/{n} succeeded ({time.time() - t0:.1f}s)\n")
    ...
    if not metrics:
        print("No results found. Verify that 05_run_ml_blo executed successfully.")
        return
    ...
    print(f"  Speedup (mean times):    {np.mean(watwaos_times) / np.mean(ml_times):8.1f}x")
    ...
    print(f"  ML outperforms Greedy:   {ml_better}/{n_eval} instances ({ml_better / n_eval * 100:.1f}%)")
    ...
    for lo, hi in bins:
        ...
        upper_str = str(hi) if hi < float("inf") else "inf"
        print(
            f"  [{lo:6} - {upper_str:>6} scenarios] "
            f"n={len(subset):3d}  "
    ...
    out_path = os.path.join(results_dir, f"batch_eval_{args.problem}_{timestamp}.pkl")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run batch evaluation of the Neur2BiLO WatwaOS surrogate model."
    )
    parser.add_argument("--problem", type=str, default="watwa_v1", help="Problem key in blo.params.")
    parser.add_argument("--n_procs", type=int, default=1, help="Number of parallel worker processes.")
    ...
    parser.add_argument("--use_context", type=int, default=0, help="Enable cross-switch-point context features.")
    parser.add_argument("--context_hidden_dim", type=int, default=32, help="Hidden dimension for context features.")
