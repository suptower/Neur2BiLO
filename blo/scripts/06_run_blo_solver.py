...
def main(args):
    ...
    fp_res = get_path(cfg.data_path, cfg, "solver_results/res_" + problem_str, suffix='pkl')
    ...
    # write mps and aux files
    print("Writing instance files:")
    solver.write_to_files(fp_mps, fp_aux)

    # call bilevel solver
    print("\nCalling Solver:")
    opt_time = time.time()
    solver.call_solver(fp_mps=fp_mps, fp_sol=fp_raw_res, solver_dir=args.solver_dir, setting=args.setting, time_limit=args.time_limit)
    ...
    # follower_obj_blo, _ = blo.solve_follower(instance_unscaled, leader_sol)
