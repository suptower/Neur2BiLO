"""
Inspects weight norms and bottleneck activations in final_instance_embedder across loop bounds.
...
def build_tensors(pml_feats, x, s):
    """Builds instance and decision tensors for a specific candidate configuration."""
    inst_features = [
        [
            pf["time_ns_high"] / 1e6,
            pf["time_ns_low"] / 1e6,
    ...
    return inst_tensor.to(DEVICE), decision_tensor.to(DEVICE)


def find_linear_layers(module, prefix=""):
    """Recursively collects all Linear layers within a module hierarchy."""
    layers = []
...
def main(args):
    ...
    print("=" * 78)
    print("CHECK A: Weight Norms in final_instance_embedder")
    print("=" * 78)
    ...
    for layer_name, layer in linear_layers:
        ...
        print(f"\n{layer_name} (in={layer.in_features}, out={layer.out_features})")
        print(
            f"  Input feature columns:  min={column_norms.min():.6f}  "
            f"median={column_norms.median():.6f}  max={column_norms.max():.6f}"
        )

        n_dead_in = int((column_norms < 1e-4).sum().item())
        print(f"  Columns with norm < 1e-4: {n_dead_in}/{layer.in_features}")

        print(
            f"  Output neurons:         min={row_norms.min():.6f}  "
            f"median={row_norms.median():.6f}  max={row_norms.max():.6f}"
        )

        n_dead_out = int((row_norms < 1e-4).sum().item())
        print(f"  Output neurons with norm < 1e-4: {n_dead_out}/{layer.out_features}")

        if layer.bias is not None:
            bias = layer.bias.detach()
            print(
                f"  Bias:                   min={bias.min():.6f}  "
                f"median={bias.median():.6f}  max={bias.max():.6f}"
            )

    print(f"\n{'=' * 78}")
    print("CHECK B: Bottleneck Activations Across loop_bound Sweep")
    print("=" * 78)

    if not linear_layers:
        print("No Linear layers found in final_instance_embedder.")
        return

    first_linear_name, first_linear = linear_layers[0]
    print(
        f"\nLayer analyzed: {first_linear_name} "
    ...
    base_pml_feats = preproc._parse_pml_features({"program_dir": args.program_dir})
    ...
    if len(candidate) != args.s:
        hook.remove()
        raise ValueError(f"Candidate configuration must contain exactly {args.s} decisions.")

    loop_bounds = np.linspace(10, 2500, args.n_steps)
    n_hidden = first_linear.out_features
    pre_activations = np.empty((args.n_steps, n_hidden), dtype=np.float32)
    ...
    try:
        for step, loop_bound in enumerate(loop_bounds):
            ...
            inst_tensor, _ = build_tensors(current_feats, candidate, args.s)
    ...
    print(f"\nSummary of {n_hidden} bottleneck neurons over {args.n_steps} loop_bound steps:")
    ...
    activation_ranges = pre_activations.max(axis=0) - pre_activations.min(axis=0)
    n_flat = int((activation_ranges < 1e-4).sum())

    print(f"\nActivation range < 1e-4:  {n_flat}/{n_hidden} ({100 * n_flat / n_hidden:.1f}%)")
    print(f"Max range:                {activation_ranges.max():.8f}")

    if n_flat == n_hidden:
        print("\n-> All bottleneck neurons remain essentially constant across inputs.")
    elif n_dead / n_hidden > 0.9:
        print("\n-> Most bottleneck neurons are permanently deactivated by ReLU.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Inspect weight norms and bottleneck pre-activations in final_instance_embedder."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Candidate decision configuration (e.g. 2 1).",
    )
